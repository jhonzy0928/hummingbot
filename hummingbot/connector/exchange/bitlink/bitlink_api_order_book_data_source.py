...
from hummingbot.connector.exchange.bitlink import bitlink_web_utils as web_utils
from hummingbot.connector.exchange.bitlink.bitlink_order_book import BitlinkOrderBook
...
if TYPE_CHECKING:
    from hummingbot.connector.exchange.bitlink.bitlink_exchange import BitlinkExchange
#0831:22:00改了行情推送的值和接口

class BitlinkAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        data = await self._connector._api_get(path_url=CONSTANTS.SNAPSHOT_PATH_URL,
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_timestamp: float = self.getTime()
        snapshot_msg: OrderBookMessage = BitlinkOrderBook.snapshot_message_from_exchange_rest(
            snapshot,
            snapshot_timestamp,
            metadata={"trading_pair": trading_pair}
        )
            # self.logger().debug(f"Saved order book snapshot1 for {trading_pair}")
        return snapshot_msg
        # else:
        #     snapshot_msg: OrderBookMessage = BitlinkOrderBook.snapshot_message_from_exchange_rest(
        #         snapshot,
        ...
        #         metadata={"trading_pair": trading_pair}
        #     )
        #     self.logger().debug(f"Saved order book snapshot2 for {trading_pair}")
        #     return snapshot_msg

    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message["params"]["symbol"])
        #有新数据订阅才会有消息
        if raw_message and "data" in raw_message:
            #for trades in raw_message["data"]:
            trade:Dict[str, Any] = raw_message["data"]
            trade_message: OrderBookMessage = BitlinkOrderBook.trade_message_from_exchange(
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        if raw_message and "data" in raw_message:
            ...
            diff_message:Dict[str, Any] = raw_message["data"]
            order_book_message: OrderBookMessage = BitlinkOrderBook.diff_message_from_exchange(
        ...
        while True:
            try:
                ...
                trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(
                    symbol=json_msg["params"]["symbol"])
                order_book_message: OrderBookMessage = BitlinkOrderBook.snapshot_message_from_exchange_websocket(
                    json_msg["data"], json_msg["data"]["t"], {"trading_pair": trading_pair})
